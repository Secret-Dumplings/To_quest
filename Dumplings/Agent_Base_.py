...
class Agent(ABC):
    ...
    def __init__(self):
        ...
        # 添加内置工具方法（如果存在且可调用）
        builtin_tools = {
            'ask_for_help': '请求其他 Agent 帮助，参数：agent_id(目标 Agent 的 UUID 或名称), message(请求内容)',
            'list_agents': '列出所有可用的 Agent 及其 UUID 和名称',
        ...
        prompt = self.prompt + tools_prompt + ", 你的 uuid " + str(self.uuid)
        ...
        # 只在 API 配置有效时进行连通性测试
        if self.api_key and self.api_provider and self.model_name:
            if not self.Connectivity():
                raise ConnectionError("请检查 api_provider，model_name，api_key")
        else:
            logger.warning("API 配置未设置，跳过连通性测试")
            ...
            builtin_tools_schema = [
                {
                    ...
                    "function": {
                        "name": "ask_for_help",
                        "description": "请求其他 Agent 帮助",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "agent_id": {"type": "string", "description": "目标 Agent 的 UUID 或名称"},
                ...
                },
                {
                    ...
                    "function": {
                        "name": "list_agents",
                        "description": "列出所有可用的 Agent 及其 UUID 和名称",
                        "parameters": {
                            "type": "object",
                            "properties": {},
                            "required": []
    ...
    def list_agents(self, **kwargs):
        """
        工具方法：返回所有可用的 Agent 列表，包括它们的 UUID 和名称
        """
        from Dumplings import agent_list

        # 获取所有唯一的 Agent（避免 UUID 和名称重复）
        unique_agents = {}
